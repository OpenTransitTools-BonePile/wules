"""
    ...
    Slightly adapted by Frank Purcell,2012
"""
...
time_formats = ['%H : %M', '%I : %M %p', '%H', '%I %p', '%I%p']

intr_date_formats_with_year = ['%m %Y', '%m %y',
                               '%d %m %Y', '%Y %m %d', '%d %B %Y', '%B %d %Y',
                                                  '%d %b %Y', '%b %d %Y',
                               '%d %m %y', '%y %m %d', '%d %B %y', '%B %d %y',
                                                  '%d %b %y', '%b %d %y']

us_date_formats_with_year = ['%m %Y', '%m %y',
                             '%m %d %Y', '%Y %d %m', '%d %B %Y', '%B %d %Y',
                                                  '%d %b %Y', '%b %d %Y',
                             '%m %d %y', '%y %m %d', '%d %B %y', '%B %d %y',
                                                  '%d %b %y', '%b %d %y']

date_formats_with_year = us_date_formats_with_year;
...
def parse_time(string):
    ...
    for format in time_formats:
        ...
        try:
            ...
            pass
        # step 2: try format with stripped spaces 
        try:
            result = time.strptime(string, format.replace(' ', ''))
            return datetime.time(result.tm_hour, result.tm_min)
        except ValueError:
            pass
...
if __name__ == '__main__':
    ...
    for test in ['12/15', '2/6/1977', '02/06/77', '1977-06-02', '2 June, 1977', 'Jun 2, 1977', '2 June', 'Jun 2']:
